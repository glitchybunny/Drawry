	...
	function createElementStream(data) {
		...
		if (!result) {
			result = new REGLElementBuffer(
				bufferState.create(null, GL_ELEMENT_ARRAY_BUFFER, true, false)._buffer
			);
		}
...
function createTextureSet(gl, extensions, limits, reglPoll, contextState, stats, config) {
	...
	function createTextureCube(a0, a1, a2, a3, a4, a5) {
		...
		function subimage(face, image, x_, y_, level_) {
			check$1(!!image, "must specify image data");
			check$1(
				typeof face === "number" && face === (face | 0) && face >= 0 && face < 6,
				"invalid face"
			);
			...
			if (info) {
				if (info.size > 1) {
					for (var j = 0; j < info.size; ++j) {
						...
						insertActiveInfo(
							uniforms,
							new ActiveInfo(name, stringStore.id(name), gl.getUniformLocation(program, name), info)
						);
					}
				}
...
function reglCore(
	...
	function parseViewportScissor(options, framebuffer, env) {
		...
		if (viewport) {
			...
			viewport = new Declaration(viewport.thisDep, viewport.contextDep, viewport.propDep, function (
				env,
				scope
			) {
	...
	function parseAttribLocations(options, attributes) {
		var staticOptions = options.static;
		var staticProgram =
			typeof staticOptions[S_FRAG] === "string" && typeof staticOptions[S_VERT] === "string";
		if (staticProgram) {
		...
		function parsePrimitive() {
			if (S_PRIMITIVE in staticOptions) {
				...
				if (isStatic(elements)) {
					...
					return new Declaration(elements.thisDep, elements.contextDep, elements.propDep, function (
						env,
						scope
					) {
		...
		function parseVertCount() {
			if (S_COUNT in staticOptions) {
				...
				if (isStatic(elements)) {
					if (elements) {
						if (OFFSET) {
							return new Declaration(OFFSET.thisDep, OFFSET.contextDep, OFFSET.propDep, function (
								env,
								scope
							) {
		...
		sortState(Object.keys(options)).forEach(function (param) {
			...
			if (GL_FLAGS[param]) {
				...
				if (isStatic(defn)) {
					...
					scope(
						env.cond(variable).then(GL, ".enable(", flag, ");").else(GL, ".disable(", flag, ");")
					);
				}
					...
					if (type === GL_SAMPLER_2D || type === GL_SAMPLER_CUBE) {
						check$1.command(
							typeof value === "function" &&
								((type === GL_SAMPLER_2D &&
									(value._reglType === "texture2d" || value._reglType === "framebuffer")) ||
									(type === GL_SAMPLER_CUBE &&
										(value._reglType === "textureCube" || value._reglType === "framebufferCube"))),
				...
				function checkVector(n, type, size) {
					if (Array.isArray(VALUE)) {
						check$1(
							...
							"must have length of " + (size === 1 ? "" : "n * ") + n
						);
					} else {
						emitCheck(
							shared.isArrayLike +
								...
								".length<=" +
								n * size,
							"invalid vector, should have length of " + (size === 1 ? "" : "n * ") + n,
							env.commandStr
						);
					}
				}
...
var createTimer = function (gl, extensions) {
	...
	function update() {
		...
		for (i = 0; i < pendingQueries.length; ++i) {
			...
			if (
				extensions.ext_disjoint_timer_query.getQueryObjectEXT(query, GL_QUERY_RESULT_AVAILABLE_EXT)
			) {
